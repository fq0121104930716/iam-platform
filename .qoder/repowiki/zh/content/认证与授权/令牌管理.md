...
## 项目结构
本项目采用多模块划分，认证相关的核心逻辑集中在 iam-auth-server 模块；通用模型与值对象位于 iam-common 模块；令牌定制、JWK配置与CAS控制器均在认证服务中实现。
...
D["CasTicketService<br/>ST生成/验证/消费"]
E["CasSloService<br/>SLO会话跟踪/清理"]
F["SsoProactiveAuthService<br/>主动授权URL构建"]
G["JwkProperties<br/>JWK路径配置"]
end
subgraph "通用模块(iam-common)"
H["TokenSettings<br/>令牌TTL设置"]
end
...
- CAS单点注销（CasSloService）：维护会话与服务映射、后向通道注销请求解析、标记前端通道注销状态、会话失效与票据清理。
...
### CAS控制器与单点注销（CasController + CasSloService）
...
## 结论
本令牌管理系统在Spring Authorization Server之上，结合自定义TokenCustomizer实现多租户与权限的动态注入；通过JWK加载与JWT解码器保障签名与验证；CAS票据服务与SLO实现单点登录与注销；主动认证服务优化用户登录后的授权体验。配合合理的TTL、密钥轮换策略与Redis分布式存储，系统在可用性、安全性与性能之间取得平衡。
