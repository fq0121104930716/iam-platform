...
E["application.yml<br/>路由/限流/跨域/监控配置"]
F["bootstrap.yml<br/>Nacos服务发现配置"]
end
A --> B
B --> C
B --> D
...
## 架构总览
下图展示了网关在 IAM 平台中的位置与交互关系：外部请求经由网关进入，网关根据路由规则转发至对应后端服务；同时在安全链路上完成 OAuth2 登录与 JWT 资源访问控制。
...
图表来源
- [application.yml:14-54](file://iam-gateway/src/main/resources/application.yml#L14-L54)
- [GatewaySecurityConfig.java:25-73](file://iam-gateway/src/main/java/iam/platform/gateway/infrastructure/security/GatewaySecurityConfig.java#L25-L73)
...
- 在 /auth/** 与 /admin/** 路由上分别配置了 RequestRateLimiter 过滤器
...
章节来源
- [application.yml:93-109](file://iam-gateway/src/main/resources/application.yml#L93-L109)
...
## 结论
iam-gateway 作为 IAM 平台的统一入口，提供了完善的路由、负载均衡、安全过滤与限流能力。通过三层安全过滤链与 JWT 资源服务器验证，确保了访问控制与权限拦截的有效性；结合 Nacos 与 Redis，实现了高可用与弹性伸缩；Actuator、Prometheus 与 Zipkin 则为运维与监控提供了坚实基础。在微服务架构中，网关承担着“门面”与“安全边界”的双重职责，是保障系统稳定与安全的关键组件。
...
- 扩展网关功能
  - 新增过滤器：在 application.yml 的 filters 中添加自定义名称与参数
