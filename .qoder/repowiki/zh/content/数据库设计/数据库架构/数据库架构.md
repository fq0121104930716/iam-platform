...
<cite>
**本文引用的文件**
- [V1__complete_schema_initialization_merged.sql](file://iam-admin-server/src/main/resources/db/V1__complete_schema_initialization_merged.sql)
- [V1__audit_schema_initialization.sql](file://iam-audit-server/src/main/resources/db/V1__audit_schema_initialization.sql)
...
## 更新摘要
**所做更改**
- 更新数据库架构文档以反映admin server数据库schema的合并：V2__admin_ui_support.sql, V3__add_audit_log_tracing_fields.sql, V5__user_credential_migration.sql等独立迁移脚本已被V1__complete_schema_initialization_merged.sql合并，简化了部署流程
- 移除了对独立迁移脚本的引用，更新为单一的合并迁移脚本
- 更新了版本管理策略，强调V1__complete_schema_initialization_merged.sql作为完整初始化脚本
- 更新了审计端架构，明确审计表结构独立于管理端
...
- **更新**：简化的数据库schema合并，V1__complete_schema_initialization_merged.sql整合了之前的独立迁移脚本
- 数据库分层设计：核心表、关联表、审计表
- **更新**：版本管理策略，强调单一的完整初始化脚本
- 架构图与表关系图
...
B["db/V1__complete_schema_initialization_merged.sql<br/>单一完整迁移脚本"]
end
subgraph "审计端配置"
...
**更新** 管理端现在使用单一的合并迁移脚本，简化了部署流程

图表来源
- [application.yml（管理端）:38-40](file://iam-admin-server/src/main/resources/application.yml#L38-L40)
- [V1__complete_schema_initialization_merged.sql:1-20](file://iam-admin-server/src/main/resources/db/V1__complete_schema_initialization_merged.sql#L1-L20)
- [V1__audit_schema_initialization.sql:1-20](file://iam-audit-server/src/main/resources/db/V1__audit_schema_initialization.sql#L1-L20)
...
- 管理端负责业务主数据与权限治理，Flyway 统一执行单一的完整初始化脚本
- **新增**：审计端提供独立的审计处理能力，包含高级监控和合规功能
- **新增**：凭证系统支持多种认证方式，提供灵活的凭据管理
- **更新**：简化的迁移脚本结构，V1__complete_schema_initialization_merged.sql整合了之前的所有独立迁移
...
T1["t_tenant"]
T2["t_user"]
T3["t_user_tenant_mapping"]
T4["t_organization"]
T5["t_user_organization_mapping"]
T6["t_application"]
T7["t_application_resource"]
T8["t_resource_permission"]
T9["t_role"]
T10["t_role_permission"]
T11["t_user_credential"]
T12["t_platform_menu"]
T13["t_tenant_menu_config"]
end
subgraph "审计端高级功能"
T14["t_audit_log"]
T15["t_alert_rule"]
T16["t_alert_record"]
T17["t_siem_endpoint"]
T18["t_compliance_report"]
end
subgraph "认证端授权表"
T19["oauth2_authorization<br/>认证端使用"]
T20["oauth2_authorization_consent<br/>认证端使用"]
end
T2 --> T3
T1 --> T3
T1 --> T4
...
**更新** 管理端使用单一的合并迁移脚本，简化了部署流程，新增平台菜单功能

图表来源
- [V1__complete_schema_initialization_merged.sql:172-510](file://iam-admin-server/src/main/resources/db/V1__complete_schema_initialization_merged.sql#L172-L510)
- [V1__audit_schema_initialization.sql:25-206](file://iam-audit-server/src/main/resources/db/V1__audit_schema_initialization.sql#L25-L206)
...
- [V1__complete_schema_initialization_merged.sql:540-581](file://iam-admin-server/src/main/resources/db/V1__complete_schema_initialization_merged.sql#L540-581)
...
章节来源
- [V1__audit_schema_initialization.sql:1-237](file://iam-audit-server/src/main/resources/db/V1__audit_schema_initialization.sql#L1-237)
...
- [Person.java:17-32](file://iam-admin-server/src/main/java/iam/platform/admin/domain/model/entity/Person.java#L17-L32)
- [TenantAccount.java:17-29](file://iam-admin-server/src/main/java/iam/platform/admin/domain/model/entity/TenantAccount.java#L17-L29)
- [V1__complete_schema_initialization_merged.sql:200-272](file://iam-admin-server/src/main/resources/db/V1__complete_schema_initialization_merged.sql#L200-L272)
...
图表来源
- [V1__complete_schema_initialization_merged.sql:26-480](file://iam-admin-server/src/main/resources/db/V1__complete_schema_initialization_merged.sql#L26-L480)
...
图表来源
- [Application.java:21-41](file://iam-admin-server/src/main/java/iam/platform/admin/domain/model/entity/Application.java#L21-L41)
- [V1__complete_schema_initialization_merged.sql:350-381](file://iam-admin-server/src/main/resources/db/V1__complete_schema_initialization_merged.sql#L350-L381)
...
图表来源
- [Organization.java:18-34](file://iam-admin-server/src/main/java/iam/platform/admin/domain/model/entity/Organization.java#L18-L34)
- [V1__complete_schema_initialization_merged.sql:277-320](file://iam-admin-server/src/main/resources/db/V1__complete_schema_initialization_merged.sql#L277-L320)
...
- [V1__audit_schema_initialization.sql:25-83](file://iam-audit-server/src/main/resources/db/V1__audit_schema_initialization.sql#L25-L83)
...
- [V1__complete_schema_initialization_merged.sql:111-140](file://iam-admin-server/src/main/resources/db/V1__complete_schema_initialization_merged.sql#L111-L140)

### 平台菜单系统（Platform Menu）
- **新增** 设计要点
  - t_platform_menu：平台功能菜单定义，支持图标、路径、排序和父子关系
  - t_tenant_menu_config：租户菜单配置，控制哪些平台菜单对特定租户可见
  - 支持仪表盘、租户管理、用户管理、组织管理、应用管理、角色权限、审计日志、会话管理、系统设置等核心功能
- 关键约束与索引
  - t_platform_menu：唯一索引（menu_code），并按 parent_id/sort_order 建索引
  - t_tenant_menu_config：唯一索引（tenant_id, menu_id），并按 tenant_id/menu_id 建索引
- 行为方法
  - 菜单创建与更新、租户菜单授权、菜单层级管理
...
PlatformMenu "1" --> "many" TenantMenuConfig : "被配置"
```

**更新** 新增平台菜单系统，支持租户级别的菜单定制

图表来源
- [V1__complete_schema_initialization_merged.sql:502-534](file://iam-admin-server/src/main/resources/db/V1__complete_schema_initialization_merged.sql#L502-L534)
...
- **新增**：菜单依赖链
  - PlatformMenu → TenantMenuConfig（菜单配置）
  - Tenant → TenantMenuConfig（租户授权）
...
PlatformMenu["t_platform_menu"] --> TenantMenuConfig["t_tenant_menu_config"]
Tenant --> TenantMenuConfig
```

**更新** 新增平台菜单系统和审计高级功能的依赖关系，包含增强的触发器逻辑
...
- 索引策略
  ...
  - **新增**：平台菜单索引 idx_platform_menu_parent 和 idx_platform_menu_sort
- 查询优化
  - 分页查询与范围筛选：优先使用复合索引覆盖
  - **新增**：告警规则查询使用 enabled 索引，提高告警引擎性能
  - **新增**：合规报告查询使用多列索引，支持复杂报表场景
  - **更新**：凭证查询使用复合索引优化主凭证查找
  - 审计日志定期清理：通过cleanup_old_audit_logs函数按保留天数删除过期记录
- 数据访问模式
...
- [V1__audit_schema_initialization.sql:59](file://iam-audit-server/src/main/resources/db/V1__audit_schema_initialization.sql#L59)
- [V1__complete_schema_initialization_merged.sql:560-567](file://iam-admin-server/src/main/resources/db/V1__complete_schema_initialization_merged.sql#L560-567)

## 故障排查指南
- Flyway 迁移失败
  - 确认迁移位置与启用状态一致：管理端启用，认证端禁用
  - 检查数据库连接参数与权限
  - **更新**：验证V1__complete_schema_initialization_merged.sql的执行顺序和依赖关系
  - **更新**：检查触发器函数update_updated_at_column是否正确创建
- 审计日志异常
  - 检查cleanup_old_audit_logs函数是否按配置运行（保留天数）
...
**更新** 新增平台菜单系统和审计高级功能的故障排查指南，包含触发器和冲突处理问题

章节来源
- [application.yml（管理端）:38-40](file://iam-admin-server/src/main/resources/application.yml#L38-L40)
- [application.yml（认证端）](file://iam-auth-server/src/main/resources/application.yml#L39)
- [V1__audit_schema_initialization.sql:230-237](file://iam-audit-server/src/main/resources/db/V1__audit_schema_initialization.sql#L230-237)
- [V1__complete_schema_initialization_merged.sql:564-567](file://iam-admin-server/src/main/resources/db/V1__complete_schema_initialization_merged.sql#L564-567)

## 结论
该数据库架构以单一的V1__complete_schema_initialization_merged.sql统一初始化为核心，结合独立的审计架构和多租户凭证管理系统，形成"业务主数据 + 协议处理 + 高级审计 + 凭证管理 + 平台菜单"的完整解决方案。通过物化路径组织树、全局与租户隔离的权限模型、完善的审计与索引策略，以及新增的实时告警、SIEM集成、合规报告和平台菜单功能，满足多租户场景下的身份、权限、安全监控、合规需求和用户体验。

**更新**：简化的数据库schema合并提供了更清晰的部署流程和更强的数据一致性保证，包含增强的触发器逻辑、精确的冲突解决机制和改进的时间戳管理。建议在生产环境进一步完善只读副本、缓存与异步审计写入，持续优化查询与清理策略，同时加强凭证安全管理和审计数据的长期保存策略。

## 附录
- 版本管理策略
  - 管理端启用 Flyway，迁移位置为 classpath:db
  - **更新**：初始版本为 V1__complete_schema_initialization_merged.sql，包含完整业务与授权表初始化
  - **更新**：认证端禁用 Flyway，避免重复迁移
  - **新增**：审计端独立的V1版本，专注于审计专用表结构
- 默认数据
  - 初始化脚本包含系统默认角色、全局权限、默认租户与管理员账户的种子数据
  - **新增**：默认告警规则包含5种常见安全场景的检测规则
  - **新增**：平台菜单系统包含9个核心功能模块的默认配置
...
- **更新**：触发器和冲突处理
  ...
  - **新增**：平台菜单的唯一约束确保菜单代码的全局唯一性

**更新** 新增平台菜单系统和审计端独立架构，包含增强的触发器和冲突处理机制

章节来源
- [application.yml（管理端）:38-40](file://iam-admin-server/src/main/resources/application.yml#L38-L40)
- [application.yml（认证端）](file://iam-auth-server/src/main/resources/application.yml#L39)
- [V1__complete_schema_initialization_merged.sql:618-824](file://iam-admin-server/src/main/resources/db/V1__complete_schema_initialization_merged.sql#L618-824)
- [V1__audit_schema_initialization.sql:227-237](file://iam-audit-server/src/main/resources/db/V1__audit_schema_initialization.sql#L227-237)