...
- 完整环境变量清单（数据库、Redis、加密密钥、Nacos服务注册）
- 服务间依赖关系、启动顺序、健康检查与故障恢复
- 生产环境高可用部署建议（负载均衡、服务发现、自动扩缩容）

## 项目结构
IAM平台采用多模块微服务架构，包含网关、认证服务、管理服务、BFF等服务，配合Nacos注册中心、PostgreSQL数据库、Redis缓存与Zipkin链路追踪进行统一部署。
...
图表来源
- [docker-compose.yml:3-190](file://docker-compose.yml#L3-L190)
...
章节来源
- [docker-compose.yml:4-190](file://docker-compose.yml#L4-L190)
...
```mermaid
flowchart TD
Start(["启动"]) --> LoadCfg["加载应用配置<br/>DB/Redis/Nacos/Zipkin"]
LoadCfg --> InitDB["初始化数据库连接池"]
InitDB --> InitRedis["初始化Redis连接"]
InitRedis --> RegisterNacos["向Nacos注册服务"]
...
- 配置差异化：
  - DEV：使用本地主机直连数据库与Redis，启用Flyway基线迁移
...
- 数据依赖：认证与管理服务依赖PostgreSQL；全部服务依赖Redis用于会话与限流
- 网关依赖：网关依赖认证服务提供的OIDC发行者URI与JWT校验
- 启动顺序：数据库与缓存优先，随后Nacos，最后业务服务；容器层面通过depends_on与健康检查保障
...
- 健康检查失败
  ...
  - 认证/管理/BFF：确认数据库与Redis连通性，检查Nacos注册状态
- 服务无法发现
...
## 结论
本部署策略以Docker Compose提供本地快速验证，以Kubernetes与Kustomization实现多环境标准化交付。通过Nacos统一服务治理、PostgreSQL与Redis提供数据与缓存支撑、Zipkin提供可观测性，结合完善的健康检查与限流策略，满足从开发到生产的全生命周期需求。生产环境建议进一步完善网络策略、安全策略与自动扩缩容策略，确保高可用与弹性伸缩。
...
章节来源
- [docker-compose.yml:73-172](file://docker-compose.yml#L73-L172)
