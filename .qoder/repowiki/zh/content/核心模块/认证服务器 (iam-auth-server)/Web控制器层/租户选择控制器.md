...
- [TenantResolutionPolicy.java](file://iam-auth-server/src/main/java/iam/platform/auth/domain/service/TenantResolutionPolicy.java)
...
D[TenantInterceptor<br/>租户拦截器]
E[TenantAwareAuthenticationToken<br/>租户感知令牌]
F[UnifiedAuthenticationSuccessHandler<br/>统一认证成功处理器]
end
...
- **selectTenant()**: 选择特定租户账户并建立完整的认证上下文
...
- [TenantContext.java:9-43](file://iam-auth-server/src/main/java/iam/platform/auth/domain/service/context/TenantContext.java#L9-L43)
...
+selectTenant(tenantAccountId, request) String
}
...
E[TenantContext]
F[TenantInterceptor]
G[TenantAwareAuthenticationToken]
H[UnifiedAuthenticationSuccessHandler]
end
subgraph "领域服务层"
I[TenantResolutionHandler]
J[TenantResolutionPolicy]
end
A --> C
B --> A
C --> E
