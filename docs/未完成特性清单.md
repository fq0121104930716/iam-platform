...
### 1.3 SAML Metadata 证书配置未完成

**位置:** `SamlMetadataGenerator.java` (第 82-85 行)

**当前状态:**
- ✅ SAML 2.0 IdP metadata 生成功能已实现
- ✅ 支持 NameID 格式和 SSO 服务配置
- ⚠️ 未实现从密钥库加载实际证书
- ⚠️ KeyDescriptor 未添加到 metadata 中
...
**实现建议:**
1. 实现从 keystore 加载 X.509 证书
2. 构建 KeyDescriptor 并添加到 IDPSSODescriptor
3. 支持证书签名验证功能
...
| 4 | SamlMetadataGenerator:82 | SAML 证书配置集成 | 可实现 | 🟡 中 | 1 天 | ⚠️ 待证书集成 |
...
8. ✅ **SAML 2.0 IdP metadata 生成** - 基础功能已实现
