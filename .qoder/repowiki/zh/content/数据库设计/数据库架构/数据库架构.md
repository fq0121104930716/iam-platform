...
## 更新摘要
**所做更改**
- 新增用户凭证系统章节，详细描述凭证表结构和管理机制
- 更新审计系统章节，增加高级功能表结构和索引设计
- 新增审计服务器架构说明，区分管理端和审计端职责
- 更新版本管理策略，包含V4和V5迁移脚本的详细说明
- 增强安全性和合规性功能，包括加密字段、告警规则和合规报告
...
## 结论
该数据库架构以 Flyway 统一初始化为核心，结合独立的审计架构和多租户凭证管理系统，形成"业务主数据 + 协议处理 + 高级审计 + 凭证管理"的完整解决方案。通过物化路径组织树、全局与租户隔离的权限模型、完善的审计与索引策略，以及新增的实时告警、SIEM集成和合规报告功能，满足多租户场景下的身份、权限、安全监控与合规需求。建议在生产环境进一步完善只读副本、缓存与异步审计写入，持续优化查询与清理策略，同时加强凭证安全管理和审计数据的长期保存策略。
...
- 默认数据
  ...
  - **新增**：凭证迁移脚本将现有密码数据转换为新的凭证表结构

**更新** 新增V4和V5版本的详细说明和审计端独立架构
