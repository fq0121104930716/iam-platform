...
> 生成时间: 2026-05-17
> 更新时间: 2026-05-23 (Admin模块部署验证: AOP修复 + 权限初始化 + Actuator端点放行)
...
| 29 | iam-admin-server | @RequirePermission 端点权限初始化数据（Flyway 脚本） | 可实现 | 🔴 高 | 1 天 | ⚠️ 待开发 |
| 30 | iam-admin-server AdminSecurityConfig | Actuator /prometheus 端点 Security 放行配置 | 可实现 | 🟡 中 | 0.5 天 | ⚠️ 待修复 |
...
### 第五阶段(Admin 模块功能完善) - 预计 2 天
1. ⚠️ @RequirePermission 端点权限初始化数据脚本 (任务 29)
2. ⚠️ Actuator /prometheus 端点 Security 放行 (任务 30)
...
16. ✅ **PermissionAuthorizationAspect AOP 修复 (2026-05-23)** - 修复多切面链中 `@annotation` 参数绑定导致的 `JoinPointMatch was NOT bound` 异常，改为手动从 MethodSignature 提取注解
...
- **2026-05-23 更新 (Admin 模块部署验证):**
  - 修复 PermissionAuthorizationAspect AOP 参数绑定 bug (Spring AOP 6.x CGLIB 代理多切面链兼容性问题)
  - 新增任务 29: @RequirePermission 端点需要 Flyway 脚本初始化租户账号权限数据
  - 新增任务 30: AdminSecurityConfig 需将 /actuator/prometheus 加入 permitAll 列表（Prometheus 抓取返回 403）
