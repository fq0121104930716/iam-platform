...
public class TenantAccountOrganizationApplicationService {
        ...
        public void assignTenantAccountToOrganization(Long tenantAccountId, Long organizationId,
                ...
                // 验证租户账号存在
                tenantAccountRepository.findById(tenantAccountId)
                ...
                // 检查是否已关联
                if (organizationMappingRepository.existsByTenantAccountIdAndOrganizationId(
                                tenantAccountId, organizationId)) {
                        throw new ConflictException(
                                        "Tenant account already belongs to this organization");
                }
                ...
                if (Boolean.TRUE.equals(isPrimary)) {
                        List<TenantAccountOrganizationMapping> existingMappings =
                                        organizationMappingRepository
                                                        .findByTenantAccountId(tenantAccountId);
                        boolean hasPrimary = existingMappings.stream()
                                        .anyMatch(m -> Boolean.TRUE.equals(m.getIsPrimary()));
                        if (hasPrimary) {
                                throw new ConflictException(
                                                "Tenant account already has a primary organization");
                        }
                }

                // 创建组织关联
                TenantAccountOrganizationMapping mapping = TenantAccountOrganizationMapping
                                .builder().tenantAccountId(tenantAccountId)
                ...
                log.info("Tenant account {} assigned to organization {}", tenantAccountId,
                                organizationId);
        }
        ...
        public void removeTenantAccountFromOrganization(Long tenantAccountId, Long organizationId) {
                ...
                organizationMappingRepository.deleteByTenantAccountIdAndOrganizationId(
                                tenantAccountId, organizationId);
                log.info("Tenant account {} removed from organization {}", tenantAccountId,
                                organizationId);
        }
        ...
        public List<OrganizationResponse> getTenantAccountOrganizations(Long tenantAccountId) {
                List<TenantAccountOrganizationMapping> mappings = organizationMappingRepository
                                .findByTenantAccountId(tenantAccountId);

                return mappings.stream()
                                .map(mapping -> organizationRepository
                                                .findById(mapping.getOrganizationId()))
                                .filter(opt -> opt.isPresent())
                                .map(opt -> toOrganizationResponse(opt.get(),
                                                mappings.stream().filter(m -> m.getOrganizationId()
                                                                .equals(opt.get().getId()))
        ...
        public List<TenantAccountResponse> getOrganizationMembers(Long organizationId) {
                ...
                return mappings.stream()
                                .map(mapping -> tenantAccountRepository
                                                .findById(mapping.getTenantAccountId()))
                                .filter(opt -> opt.isPresent())
                                .map(opt -> toTenantAccountResponse(opt.get()))
                                .collect(Collectors.toList());
        }
        ...
        public void setPrimaryOrganization(Long tenantAccountId, Long organizationId) {
                ...
                TenantAccountOrganizationMapping mapping = organizationMappingRepository
                                .findByTenantAccountIdAndOrganizationId(tenantAccountId,
                                                organizationId)
                                .orElseThrow(() -> new IllegalArgumentException(
                                                "Tenant account does not belong to this organization"));
        ...
        private OrganizationResponse toOrganizationResponse(Organization org,
                ...
                return OrganizationResponse.builder().id(org.getId()).tenantId(org.getTenantId())
                                .orgCode(org.getOrgCode()).orgName(org.getOrgName())
                                .orgType(org.getOrgType()).parentId(org.getParentId())
                                .level(org.getLevel()).path(org.getPath())
                                .sortOrder(org.getSortOrder()).managerId(org.getManagerId())
                                .phone(org.getPhone()).email(org.getEmail())
                                .status(org.getStatus() != null ? org.getStatus().name()
                                                : OrgStatus.ACTIVE.name())
                                .description(org.getDescription()).children(new ArrayList<>())
                                .memberCount(0).isPrimary(isPrimary).position(position)
        ...
        private TenantAccountResponse toTenantAccountResponse(TenantAccount account) {
                return TenantAccountResponse.builder().id(account.getId())
                                .userId(account.getUserId()).tenantId(account.getTenantId())
                                ...
                                .timezone(account.getTimezone()).createdAt(account.getCreatedAt())
