...
## 更新摘要
**变更内容**
- 简化了租户感知过滤器的职责，移除了Session恢复逻辑
- 明确了API优先认证流程，通过网关头传递租户上下文
- 优化了过滤器链顺序，统一认证过滤器在前，租户感知过滤器在后
- 强化了网关头上下文传递机制，支持API优先认证场景
...
- [UnifiedAuthenticationFilter.java:18-38](file://iam-auth-server/src/main/java/iam/platform/auth/interfaces/web/filter/UnifiedAuthenticationFilter.java#L18-L38)
- [CompositeAuthenticationProvider.java:26-68](file://iam-auth-server/src/main/java/iam/platform/auth/application/service/CompositeAuthenticationProvider.java#L26-L68)
- [TenantAwareAuthenticationFilter.java:23-44](file://iam-auth-server/src/main/java/iam/platform/auth/interfaces/web/filter/TenantAwareAuthenticationFilter.java#L23-L44)
...
- **租户感知过滤器（TenantAwareAuthenticationFilter）**
  - **更新**：仅负责从网关HTTP头提取租户上下文，不再参与Session恢复逻辑，支持API优先认证流程。
...
- [CompositeAuthenticationProvider.java:26-75](file://iam-auth-server/src/main/java/iam/platform/auth/application/service/CompositeAuthenticationProvider.java#L26-L75)
- [UnifiedAuthenticationSuccessHandler.java:28-67](file://iam-auth-server/src/main/java/iam/platform/auth/interfaces/web/handler/UnifiedAuthenticationSuccessHandler.java#L28-L67)
- [UnifiedAuthenticationFailureHandler.java:13-36](file://iam-auth-server/src/main/java/iam/platform/auth/interfaces/web/handler/UnifiedAuthenticationFailureHandler.java#L13-L36)
- [TenantAwareAuthenticationFilter.java:23-68](file://iam-auth-server/src/main/java/iam/platform/auth/interfaces/web/filter/TenantAwareAuthenticationFilter.java#L23-L68)
...
- [UnifiedAuthenticationToken.java:17-68](file://iam-auth-server/src/main/java/iam/platform/auth/domain/model/valueobject/UnifiedAuthenticationToken.java#L17-L68)

## 架构总览
统一认证过滤器替换默认表单登录过滤器，将所有第一方认证（密码、短信、邮箱、LDAP）与OAuth2社交登录汇聚到同一入口。认证成功后，统一成功处理器触发后认证流水线并依据协议路由决定最终跳转地址；失败则统一重定向到带错误参数的登录页。**更新**：租户感知过滤器现在专门负责从网关HTTP头提取租户上下文，支持API优先认证流程，确保后续业务逻辑的多租户隔离。令牌定制器在生成JWT时注入多租户上下文与权限信息。

```mermaid
sequenceDiagram
participant U as "用户浏览器/API客户端"
...
- [CompositeAuthenticationProvider.java:32-68](file://iam-auth-server/src/main/java/iam/platform/auth/application/service/CompositeAuthenticationProvider.java#L32-L68)
- [UnifiedAuthenticationSuccessHandler.java:36-66](file://iam-auth-server/src/main/java/iam/platform/auth/interfaces/web/handler/UnifiedAuthenticationSuccessHandler.java#L36-L66)
- [UnifiedAuthenticationFailureHandler.java:22-35](file://iam-auth-server/src/main/java/iam/platform/auth/interfaces/web/handler/UnifiedAuthenticationFailureHandler.java#L22-L35)
...
**图表来源**
- [UnifiedAuthenticationFilter.java:31-71](file://iam-auth-server/src/main/java/iam/platform/auth/interfaces/web/filter/UnifiedAuthenticationFilter.java#L31-L71)
...
### 租户感知认证过滤器与上下文
- **设计要点**
  - **更新**：仅负责从网关HTTP头提取租户上下文，不再参与Session恢复逻辑。
  - 在请求结束时清理ThreadLocal，避免内存泄漏。
  - 支持API优先认证流程，通过标准HTTP头传递租户信息。
- **上下文传递**
  - TenantContext提供当前人员、租户与租户账户ID的线程本地存储。
  - 租户感知过滤器在每次请求开始时从网关头恢复这些值，确保后续业务逻辑可见。

```mermaid
flowchart TD
Enter(["请求进入"]) --> CheckHeader{"检查网关头是否存在？"}
CheckHeader -- 是 --> Restore["从网关HTTP头恢复租户上下文"]
CheckHeader -- 否 --> Chain["继续执行过滤链"]
Restore --> Chain
...
**图表来源**
- [TenantAwareAuthenticationFilter.java:28-44](file://iam-auth-server/src/main/java/iam/platform/auth/interfaces/web/filter/TenantAwareAuthenticationFilter.java#L28-L44)
- [TenantContext.java:7-45](file://iam-common/src/main/java/iam/platform/common/context/TenantContext.java#L7-L45)

**章节来源**
- [TenantAwareAuthenticationFilter.java:23-68](file://iam-auth-server/src/main/java/iam/platform/auth/interfaces/web/filter/TenantAwareAuthenticationFilter.java#L23-L68)
- [TenantContext.java:1-45](file://iam-common/src/main/java/iam/platform/common/context/TenantContext.java#L1-L45)
...
- **网关头上下文传递**
  - **新增**：所有下游服务通过GatewayHeaderContextFilter从网关HTTP头提取租户上下文，支持API优先认证。
...
GHC["GatewayHeaderContextFilter"] --> TC
```
...
- [UnifiedAuthenticationSuccessHandler.java:30-31](file://iam-auth-server/src/main/java/iam/platform/auth/interfaces/web/handler/UnifiedAuthenticationSuccessHandler.java#L30-L31)
...
- **过滤器链顺序**
  - **更新**：统一认证过滤器替换默认表单过滤器，租户感知过滤器在统一认证之后，减少重复上下文设置开销。
  - **新增**：网关头上下文过滤器在租户感知过滤器之前，确保API请求的租户上下文优先处理。
...
- **租户上下文丢失**
  - **更新**：确认网关正确设置了标准HTTP头（X-User-Id、X-Tenant-Id等），租户感知过滤器会在请求结束清理ThreadLocal，避免内存泄漏。
- **API认证问题**
  - **新增**：检查API客户端是否正确传递网关头，确认租户上下文在请求到达下游服务时能够被正确提取。

**章节来源**
- [UnifiedAuthenticationFailureHandler.java:15-35](file://iam-auth-server/src/main/java/iam/platform/auth/interfaces/web/handler/UnifiedAuthenticationFailureHandler.java#L15-L35)
...
- [TenantAwareAuthenticationFilter.java:38-43](file://iam-auth-server/src/main/java/iam/platform/auth/interfaces/web/filter/TenantAwareAuthenticationFilter.java#L38-L43)

## 结论
该安全过滤器模块通过统一认证入口、复合认证策略与租户上下文管理，实现了多认证方式的一致化与多租户隔离。**更新**：简化后的租户感知过滤器专注于网关头上下文提取，支持API优先认证流程，提高了系统的灵活性和可扩展性。配合令牌定制器与协议路由，既能满足传统表单登录，也能兼容OAuth2与LDAP等企业级需求。通过预认证流水线与限流封禁策略，系统在安全性与可用性之间取得平衡。

## 附录：配置示例与最佳实践
- **安全配置要点**
  - 启用统一认证过滤器替换默认表单登录过滤器，确保所有认证走同一入口。
  - OAuth2登录页面与用户信息服务配置正确，成功处理器指向统一成功处理器。
  - **更新**：租户上下文现在通过网关HTTP头传递，不再依赖Session存储。
- **认证策略启用**
...
- **API优先认证配置**
  - **新增**：确保网关正确设置标准HTTP头（X-User-Id、X-Tenant-Id、X-Tenant-Account-Id等）。
  - 下游服务通过TenantAwareAuthenticationFilter自动提取租户上下文，无需额外配置。
- **性能优化建议**
  - 调整数据库连接池参数与Redis连接配置，结合监控指标持续优化。
  - 对权限加载进行缓存或批量预热，降低令牌生成时延。
  - 合理设置限流与封禁阈值，平衡安全与用户体验。
  - **新增**：优化网关头上下文提取性能，避免频繁的HTTP头解析操作。

**章节来源**
- [DefaultSecurityConfig.java:35-62](file://iam-auth-server/src/main/java/iam/platform/auth/infrastructure/config/DefaultSecurityConfig.java#L35-L62)
- [application.yml:54-127](file://iam-auth-server/src/main/resources/application.yml#L54-L127)
- [TokenCustomizer.java:33-61](file://iam-auth-server/src/main/java/iam/platform/auth/infrastructure/security/TokenCustomizer.java#L33-L61)
- [TenantAwareAuthenticationFilter.java:15-26](file://iam-auth-server/src/main/java/iam/platform/auth/interfaces/web/filter/TenantAwareAuthenticationFilter.java#L15-L26)