...
public class TenantAccountRoleApplicationService {
        ...
        public void assignRoleToTenantAccount(Long tenantAccountId, Long roleId) {
                ...
                roleRepository.findById(roleId).orElseThrow(
                                () -> new RoleNotFoundException("Role not found: " + roleId));

                // 检查是否已分配
                if (roleMappingRepository.existsByTenantAccountIdAndRoleId(tenantAccountId,
                                roleId)) {
                        throw new IllegalArgumentException(
                                        "Role already assigned to tenant account");
                }

                // 创建角色映射
                TenantAccountRoleMapping mapping = TenantAccountRoleMapping.builder()
                                .tenantAccountId(tenantAccountId).roleId(roleId)
                                .assignedAt(LocalDateTime.now())
                                .assignedBy(TenantContext.getCurrentUserId() != null
                                                ? "User-" + TenantContext.getCurrentUserId()
                                                : "system")
                                .build();
        ...
        public List<RoleResponse> getTenantAccountRoles(Long tenantAccountId) {
                ...
                return mappings.stream()
                                ...
                                .map(opt -> toRoleResponse(opt.get())).collect(Collectors.toList());
        }
        ...
        public List<PermissionResponse> getTenantAccountPermissions(Long tenantAccountId) {
                ...
                // 收集所有角色的权限并去重
                Set<Long> permissionIds = mappings.stream()
                                .flatMap(mapping -> rolePermissionRepository
                                                .findByRoleId(mapping.getRoleId()).stream())
                ...
                return permissionIds.stream()
                                ...
                                .filter(opt -> opt.isPresent())
                                .map(opt -> toPermissionResponse(opt.get()))
        ...
        private RoleResponse toRoleResponse(Role role) {
                return RoleResponse.builder().id(role.getId()).tenantId(role.getTenantId())
                                ...
                                .roleType(role.getRoleType()).description(role.getDescription())
                                .isSystem(role.getIsSystem()).createdAt(role.getCreatedAt())
